...
## Работа с дата пайплайнами

В качестве инструмента используется Airflow. В качестве пакетного менеджера внутри Airflow используется pip, поскольку poetry имеет низкую совместимость с Airflow. 
Для разрешения зависимостей используется poetry, а затем генерируется `requirements.txt` с помощью [дополнения](https://github.com/python-poetry/poetry-plugin-export) для poetry. 

### Описание Dag

Состоит из трех этапов:

1. Получение данных из LakeFS
2. Обучение модели
3. Подсчет метрик на валидационном датасете
